...
namespace Perpustakaan_IF19C.GUI
{
    public partial class Fpengembalian : Form
    {
        ...
        private void simpan_detailpengembalian()
        {
            ...
            {
                conn.Open();
                int daysOverdue = (int)(DtpTanggalPengembalian.Value - DtpTanggalPeminjaman.Value).TotalDays;
                if (daysOverdue > 7)
                {
                    decimal fixedFineAmount = 2000;

                    cmd = new SqlCommand("EXECUTE simpan_detailpengembalian '" + TxtIDPengembalian.Text + "','" + TxtIDBuku.Text + "','" +
                        TxtBukudiKembalikan.Text + "','" + TxtDenda.Text + "','" + fixedFineAmount + "'", conn);
                }
                else
                {
                    cmd = new SqlCommand("EXECUTE simpan_detailpengembalian '" + TxtIDPengembalian.Text + "','" + TxtIDBuku.Text + "','" +
                        TxtBukudiKembalikan.Text + "','" + TxtDenda.Text + "','0'", conn);
                }
        ...
        private void BtnSimpan_Click(object sender, EventArgs e)
        {
            ...
            FCetakPengembalian cp = new FCetakPengembalian();
            cp.Show();
            awal();
        }
